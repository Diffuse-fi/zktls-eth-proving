...
use crate::{
    attestation_data::{SlotProofData, SlotsProofPayload},
    eth::{
...
pub(crate) fn construct_report_data(payload: &SlotsProofPayload) -> Result<[u8; 64]> {
...
pub(crate) fn parse_slots_to_prove(slots_str: &str) -> Result<Vec<B256>> {
    let slot_numbers: Result<Vec<U256>, _> = slots_str
        .split(',')
        .map(|s| {
            let s = s.trim();
            U256::from_str(s)
    ...
    for slot_number in slot_numbers {
        let slot_key: B256 = slot_number.0.to_be_bytes().into();
